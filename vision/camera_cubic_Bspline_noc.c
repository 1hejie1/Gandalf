...
#define LAST_DISTANCE_THRES 0.01F
   if(support->last_Xdu != NULL && support->last_Xdu[0].x < 1.0e8
      && fabs(support->last_Xdu[0].x-Xd->x)<LAST_DISTANCE_THRES
      && fabs(support->last_Xdu[0].y-Xd->y)<LAST_DISTANCE_THRES)
   {
      /* use the last position */
      *Xu = support->last_Xdu[1];
      bFound = GAN_TRUE;
   }
   else if(support->last_Xdu == NULL)
   {
      /* test whether we are close to one of the corners */
      (void)CAM_VEC2_SUB_Q(Xd, &support->warped_pos[1][1], &d);
      dist2 = CAM_SQR(d.x) + CAM_SQR(d.y);
      if(dist2 < 0.00001F)
      {
         Xu->x = -CAM_HALF;
         Xu->y = -CAM_HALF;
      ...
      if(!bFound)
      {
         (void)CAM_VEC2_SUB_Q(Xd, &support->warped_pos[block_limit-1][1], &d);
         dist2 = CAM_SQR(d.x) + CAM_SQR(d.y);
         if(dist2 < 0.00001F)
         {
            Xu->x = -CAM_HALF;
      ...
      if(!bFound)
      {
         (void)CAM_VEC2_SUB_Q(Xd, &support->warped_pos[1][block_limit-1], &d);
         dist2 = CAM_SQR(d.x) + CAM_SQR(d.y);
         if(dist2 < 0.00001F)
         {
            Xu->x =  CAM_HALF;
      ...
      if(!bFound)
      {
         (void)CAM_VEC2_SUB_Q(Xd, &support->warped_pos[block_limit-1][block_limit-1], &d);
         dist2 = CAM_SQR(d.x) + CAM_SQR(d.y);
         if(dist2 < 0.00001F)
         {
            Xu->x = CAM_HALF;
   ...
   if(!bFound)
   {
      ...
      int r, c;
      for(r=1; r<block_limit; r++)
         for(c=1; c<block_limit; c++)
         {
            (void)CAM_VEC2_SUB_Q(Xd, &support->warped_pos[r][c], &d);
            (void)CAM_MAT22_MULTV2_Q(&support->warped_deriv[r][c], &d, &v);
            dist2 = CAM_SQR(v.x) + CAM_SQR(v.y);
            if(support->warped_deriv[r][c].xx <= CAM_ZERO || support->warped_deriv[r][c].yy <= CAM_ZERO)
               dist2 += CAM_HALF;
...
#define PROJECTION_THRESHOLD 0.001
   ...
   /* store this position for next pixel if it is within the mesh */
   if(support->last_Xdu != NULL && Xu->x >= -CAM_HALF && Xu->x <= CAM_HALF && Xu->y >= -CAM_HALF && Xu->y <= CAM_HALF)
   {
      support->last_Xdu[0] = *Xd;
      support->last_Xdu[1] = *Xu;
   }
